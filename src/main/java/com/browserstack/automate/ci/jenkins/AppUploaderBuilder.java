...
public class AppUploaderBuilder extends Builder {
  ...
  public boolean perform(@Nonnull AbstractBuild<?, ?> build, @Nonnull Launcher launcher,
    ...
    if (credentials == null) {
      PluginLogger.logDebug(listener.getLogger(),
          "Was not able to fetch credentials in AppUploadBuilder.");
      return false;
    }
    ...
    try {
      PluginLogger.log(listener.getLogger(),
          "Uploading app " + this.buildFilePath + " to Browserstack.");
      String appId = appUploader.uploadFile();
      PluginLogger.log(listener.getLogger(),
          this.buildFilePath + " uploaded successfully to Browserstack with app_url : " + appId);
